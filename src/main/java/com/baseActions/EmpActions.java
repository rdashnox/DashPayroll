...
public class EmpActions {
    ...
    /* public List<EmpDetails> searchPayrollSummary(String keyword) {
        ...
        try {
            ...
            while (rs.next()) {
                list.add(new EmpDetails(
                        ...
                        rs.getString(searchQuery.position),
                        rs.getString(searchQuery.status),
                        rs.getString(searchQuery.birthDay),
                        rs.getString(searchQuery.immediateSupervisor),
                        rs.getString(searchQuery.phoneNumber),
                        rs.getString(searchQuery.designation),
                        rs.getString(searchQuery.email),
...
*/
    ...
    // Method used to display the user info to the Payslip JFrame. This was never used.
    /*
    public void displayEmployeeInfo(JLabel jLabel53SSSN, JLabel jLabel5PHN, JLabel jLabel55PN, JLabel jLabel45TINN) {
        try {
            PreparedStatement ps = DBConnection.con.prepareStatement(searchQuery.displayEmployeeInfo);
